...
class  ServidorHttp
{
    ...
    public ServidorHttp (int porta = 8080){
        ...
        try
        {
            ...
            Task servidorHttpTaks = Task.Run(()=> AguardarRequests());
            servidorHttpTaks.GetAwaiter().GetResult();
        }
        catch (Exception e)
        {
            ...
            if(textoRequisicao.Length > 0)
            {
                ...
                if (fiArquivo.Exists){
                    ...
                    if (TiposMime.ContainsKey(fiArquivo.Extension.ToLower()))
                    {
                        ...
                        byteCabecalho = GerarCabecalho(versaoHttp, tipoMime, 
                            "200", bytesConteudo.Length);
                    }
                    else
                    {
                        bytesConteudo = Encoding.UTF8.GetBytes(
                            "<h1>Error 415 - Tipo de arquivo não suportado.</h1>"
                        );
                        byteCabecalho = GerarCabecalho(versaoHttp, "text/html;charset=uft-8", "415", bytesConteudo.Length);
                    }
                }
                else
                {
                     bytesConteudo = Encoding.UTF8.GetBytes(
                        "<h1>Error 404 - Arquivo Nao Encontrado</h1>");
                    byteCabecalho = GerarCabecalho(versaoHttp, "text/html;charset=uft-8", 
                        "200", bytesConteudo.Length);
                } 

                int byteEnviados = conexao.Send(byteCabecalho, byteCabecalho.Length, 0);
                byteEnviados += conexao.Send(bytesConteudo, bytesConteudo.Length, 0);
                conexao.Close();
                Console.WriteLine($"\n{byteEnviados} bytes enviados em resposta à requisicao #{numeroRequest}.");
            }
        }
    ...
    private void PopularDiretoriosHosts()
    {
        ...
        this.DiretoriosHosts.Add("localhost","E:\\Usuários\\jorge.nascimento\\Desktop\\ServidorWEB\\www\\localhost");
        this.DiretoriosHosts.Add("mateus.com", "E:\\Usuários\\jorge.nascimento\\Desktop\\ServidorWEB\\www\\mateus.com");
    }
